...
export class ApiService {
  ...
  getPage<T>(path: string, params?: Record<string, string | number | boolean | undefined>): Observable<ApiResponse<T[]>> {
    return this.http.get<ApiResponse<T[]>>(`${this.baseUrl}${path}`, { params: this.toParams(params) });
  }
  ...
  delete<T>(path: string, params?: { params: { role: string; }; }): Observable<ApiResponse<T>> {
    return this.http.delete<ApiResponse<T>>(`${this.baseUrl}${path}`, { params: this.toParams(params?.params) });
  }
