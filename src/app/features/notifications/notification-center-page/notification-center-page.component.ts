import { ChangeDetectionStrategy, Component, ElementRef, inject, OnInit, OnDestroy, AfterViewInit, afterNextRender, signal, computed, viewChild } from '@angular/core';
...
export class NotificationCenterPageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  protected confirmSlot(): void {
    ...
    this.emergencyService.accept(data.emergencyId, slotId).subscribe({
      next: () => {
        if (slotId) {
  ...
  protected formatSlotTime(slot: Slot): string {
    return `${slot.startTime}  ${slot.endTime}`;
  }
