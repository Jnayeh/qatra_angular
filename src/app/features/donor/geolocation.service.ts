...
export class GeolocationService {
  ...
  private readonly TRACKING_INTERVAL_MS = 300_000;
  ...
  private fetchAndUpdatePosition(): void {
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        if (prev && this.haversine(prev.latitude, prev.longitude, latitude, longitude) < this.MIN_DISTANCE_METERS) {
          return;
        }
        this.lastPosition.set({
          latitude,
        ...
        this.permissionStatus.set('granted');
        this.donorService.updateLocation({ latitude, longitude }).subscribe();
      },
      (error) => {
